...
public class TeamListEntryView {
	...
	public static void readTeamListEntryView(String customizedDir) {
		...
		if (m.find()) {
			...
			settings.setRaid_hp_width(width);
			settings.setRaid_hp_height(height);
		}

		// 헬스바 알파값추출
		settings.setRaidHPAlpha(1.0);
		p = Pattern.compile("<CharStatBar\\s+name=\"HealthBar\"[\\s\\S]*?view_local_alpha=\"([0-9]*\\.?[0-9]+)\"");
		m = p.matcher(content);

		if (m.find()) {
			double alpha = Double.parseDouble(m.group(1));
			settings.setRaidHPAlpha(alpha);
		}

		// 네임 bg 알파값추출
		settings.setRaid_nameBg_alpha(1);
		p = Pattern.compile(
		...
		if (m.find()) {
			double alpha = Double.parseDouble(m.group(1));
			settings.setRaid_nameBg_alpha(alpha);
		}
	...
	public static void writeTeamListEntryView(String customizedDir) {
		...
		// 사이즈
		int width = settings.getRaid_hp_width();
		int height = settings.getRaid_hp_height();
		int text_width;
		String left_margin;
		String right_margin;
		...
		// hp 알파
		if (settings.getRaidHPAlpha() == 1.0) {
			content = content.replaceAll("var_hp_alpha", "");
		} else {
			content = content.replaceAll("var_hp_alpha",
					"view_local_alpha=\"" + String.valueOf(settings.getRaidHPAlpha()) + "\"");
		}

		// 네임 bg 알파
		if (settings.getRaid_nameBg_alpha() == 1.0) {
			content = content.replaceAll("var_nameBg_alpha", "");
		} else {
			content = content.replaceAll("var_nameBg_alpha",
					" view_local_alpha=\"" + String.valueOf(settings.getRaid_nameBg_alpha()) + "\"");
		}
		...
		if (font_size == 0) {
			...
			nameBg_height = "21";
			name_layout_borders="layout_borders=\"Rect(3,-2,0,0)\"";
		}
